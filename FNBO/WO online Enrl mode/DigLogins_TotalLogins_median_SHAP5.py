...
# Save the plot
with open("shap_force_plot_logins_semifinal1.html", "w") as f:
...
########################## Preppind for DBScan CLustering ##################
...
# Assuming you chose an eps value based on the elbow, for demonstration let's say it's 0.5
chosen_eps = 0.25
...
# Create a list of colors for the discrete sequence. 
# Adjust the list based on the number of clusters you have.
...
# viridis_colors = plt.cm.viridis(np.linspace(0, 1, len(np.unique(s_labels))))

# # Convert matplotlib colors to the format acceptable by Plotly (RGBA)
# viridis_discrete = [f'rgba({int(r*255)},{int(g*255)},{int(b*255)},{a})' for r, g, b, a in viridis_colors]

# # Now use viridis_discrete in your Plotly plot
# fig = px.scatter(df_embedding, 
#                  x='UMAP 1st Dimension', 
#                  y='UMAP 2nd Dimension', 
#                  color='Cluster',
#                  color_discrete_sequence=viridis_discrete,
#                  labels={'Cluster': 'DBSCAN Cluster'},
#                  title='UMAP of SHAP Values with DBSCAN Clusters')
...
# Calculate quartiles of your target variable
quartiles = np.percentile(y_test, [25, 50, 75])
...
# # Then use the processed data with SkopeRules
# for cluster in np.unique(s_labels):
#     for q in range(4):
#         # Create binary target for individual cluster and quartile
...
#         sr = SkopeRules(max_depth=2).fit(X_test_processed, yc)

#         # Check if any rules were found
#         if sr.rules_:
#             # Print the cluster, quartile, and best decision rule
#             print(f"Cluster: {cluster}, Quartile: {q}")
#             print("Rule:", sr.rules_[0][0])

#             # Print precision and recall of the best decision rule
...
# # Get the column names from your processed DataFrame
...
#             # Print the cluster, quartile, and best decision rule with column names
#             print(f"Cluster: {cluster}, Quartile: {q}")
#             print("Rule:", rule_with_names)
...
# Filter the rules based on precision and recall criteria and display
